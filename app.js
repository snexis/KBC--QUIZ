...
function processVoiceCommand(cmd) {
    ...
    const mappings = {
        'a': 0, '?': 0, '??': 0, 'one': 0, '?????': 0, 'option a': 0, '?': 0, '1': 0, '?': 0,
        'b': 1, '??': 1, '???': 1, 'two': 1, '????????': 1, 'option b': 1, '?': 1, '2': 1, '?': 1,
        'c': 2, '??': 2, '???': 2, 'three': 2, '??????': 2, 'option c': 2, '?': 2, '3': 2, '?': 2,
        'd': 3, '??': 3, '???': 3, 'four': 3, '??????': 3, 'option d': 3, '?': 3, '4': 3, '?': 3
    };
...
function showComingSoonScreen() {
    ...
    if (nextBtn) {
        nextBtn.innerText = curLang === 'bn' ? '????? ????? ??' : 'Change Subject ??';
...
function loadQ() {
    ...
    if (curIdx > 0 && curIdx % 5 === 0 && currentSlabCount !== curIdx) {
        ...
        const nextBtn = document.querySelector('#scr-slab-cleared .btn');
        if (nextBtn) {
            nextBtn.innerText = curLang === 'bn' ? '??????? ?????? ??? (Next Slab) ??' : 'Next Slab ??';
